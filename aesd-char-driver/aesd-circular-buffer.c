...
bool buffer_max_size_exceeded(uint8_t offset) {
  return offset > AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED - 1;
}

void print_buffer(struct aesd_circular_buffer *buffer, uint8_t offset) {

  printf("buffer [%i]: %s", (int)buffer->entry[offset].size,
         buffer->entry[offset].buffptr);
}
...
struct aesd_buffer_entry *aesd_circular_buffer_find_entry_offset_for_fpos(
  ...
  for (int i = 0; i < AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED; i++) {
    size_t buffer_entry_size = buffer->entry[out_offs].size;

    if (buffer_entry_size == 0)
      return NULL;

    if (char_offset > buffer_entry_size - 1) {
      out_offs++;
      if (buffer_max_size_exceeded(out_offs)) {
        out_offs = 0;
      }
      char_offset -= buffer_entry_size;
      continue;
    }
...
void aesd_circular_buffer_add_entry(struct aesd_circular_buffer *buffer,
  ...
  if (buffer->full) {
    ...
    if (buffer_max_size_exceeded(buffer->out_offs)) {
      buffer->out_offs = 0;
    }
  }
  ...
  buffer->in_offs++;
  if (buffer_max_size_exceeded(buffer->in_offs)) {
    buffer->in_offs = 0;
  }
